...
- **Display name = `BlockX`** (manifest `android:label`, service label, `strings.xml` app_name, Flutter `MaterialApp.title` + AppBar).
- **Identity is now `blockx` everywhere:** applicationId `com.blockx.app`; Dart package `blockx`; Kotlin package/namespace `com.example.blockx`; MethodChannel `com.blockx.app/blocker`; block-screen taskAffinity `com.blockx.app.blockscreen`. (These were renamed from the original `com.block.app` / `block` / `com.example.block` at the owner's explicit request — the whole project is "BlockX" now.) Because applicationId changed, Android treats a rebuilt APK as a **brand-new app**: uninstall the old one, re-grant all 3 permissions (Accessibility, overlay, Usage access), and re-add the block list. Don't casually change these again for the same reason.
- **The domain word "block" (meaning *blocking apps*) stays** — class names (`BlockActivity`, `BlockRepository`, `AppBlockerService`), file names (`block_store.dart`), and storage keys (`blocklist_v2`, `block_prefs`, `configs_json`) describe the feature, not the app's identity, and are NOT renamed to "blockx".
...
- `services/block_platform.dart` — the only Dart↔native bridge (`MethodChannel com.blockx.app/blocker`): `getInstalledApps`, `setConfigs`, and permission check/open methods.
...
6. **Anti-flicker for aggressive apps.** Some apps (e.g. Facebook) re-launch themselves to the foreground and ping-pong with the block screen. `showBlockScreen` detects the *same* package reappearing within 4s and fires `GLOBAL_ACTION_HOME` to decisively background it before showing the block screen (an app can't beat Home). Normal apps/games block on the first try and never trigger the Home kick (so no extra launcher flash). Guarded by `BlockActivity.isVisible` + a 400ms cooldown so it can't stack/spam. **When the Home kick fires, the block screen is launched ~350ms LATER (`launchBlockActivity` posted on the handler), not immediately.** Launching right after `GLOBAL_ACTION_HOME` raced the Home transition (and a still-finishing `singleTask` block screen), which left the app closed but **no block screen visible** — this happened specifically when the user dismissed the block screen with our in-app "Go to home" button and reopened the same app within the 4s window. The non-kick path still launches immediately.
7. **Time-limited mode (usage quota).** Per app: `opensPerDay` + `sessionMinutes`. Flow: open a timed app → interstitial "Is this really needed?" (Open disabled 5s) → Open spends one daily open + starts a session + relaunches the app → a floating widget (shows **BlockX's own launcher icon**, not the blocked app's; tap = expand to opens-left / time-left / **End now**) shows during the session → when the session time runs out the 350ms poll re-decides and blocks again → when the day's opens are used up it's fully blocked until midnight (opens reset on date change). Decisions all live in `BlockRepository.decide()`. The widget is **draggable**: drag it anywhere and on release it snaps to the nearest left/right screen edge (portrait or landscape). Its edge + vertical position are remembered for the service's lifetime. **Never mutate the live overlay's view hierarchy** (e.g. `removeAllViews()`/re-`addView` on the attached row to reorder children) — doing so left a null child in the `ViewGroup` and crashed the whole process with an `NPE in ViewGroup.newDispatchApplyWindowInsets` during insets dispatch, killing the AccessibilityService and disabling all blocking. Keep the child order fixed (`panel` then `icon`) and only reposition via `updateViewLayout` (`params.x`), which `anchorToEdge()` does to keep the badge on-screen at either edge.
...
- **realme kills the accessibility service / turns Accessibility off by itself.** ColorOS aggressive battery/background management sometimes kills the app's process (seen in logcat as `MainActivity … app died, no saved state` with a new pid, and NO `AndroidRuntime` crash) — when that happens realme often flips the Accessibility toggle off, so blocking silently stops until it's re-enabled. This is not a BlockX crash; verify by checking `adb logcat -b crash` is clean. Mitigate on-device: lock BlockX in Recents, and Settings → Battery/App-battery-management → BlockX → allow background + auto-launch, don't optimize. No app-side code can stop the OS from disabling an accessibility service.
